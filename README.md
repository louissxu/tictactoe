...
CSS is written in SCSS and compiled to plain CSS using node-sass via a VS Code build script.
...
Symbols can be emoji for visual variety, rather than just X and O in regular font. Debated whether to embed/hardcode an emoji font or embed svg/images directly. However, the decision was made to avoid this. Best solution likely involves webpack, twemoji or similar and was beyond the scope of this project. Accepted that emoji presentation between devices will be non-uniform and in devices without up-to-date native emoji support there might be tofu for the unrecognised emoji.
